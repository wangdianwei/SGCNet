...
class DWConvBNAct(nn.Module):
    ...
    def __init__(self, c, k=3, s=1, p=None, dil=1, act=True):
        ...
        if p is None:
            p = (k // 2) * dil
        self.dw = ConvBNAct(c, c, k=k, s=s, p=p, g=c, act=act)
        self.pw = ConvBNAct(c, c, k=1, s=1, p=0, g=1, act=act)
...
class CrackExpert(nn.Module):
    ...
    def __init__(self, c, k_strip=7):
        ...
        self.h = ConvBNAct(c, c, k=(1, k_strip), p=(0, k_strip // 2), g=c)
        self.v = ConvBNAct(c, c, k=(k_strip, 1), p=(k_strip // 2, 0), g=c)
...
class PatchRepairExpert(nn.Module):
    ...
    def __init__(self, c, k_big=9):
        ...
        self.net = nn.Sequential(
            ConvBNAct(c, c, k=1),
            ConvBNAct(c, c, k=k_big, p=k_big // 2, g=c),  # DW big kernel
            ConvBNAct(c, c, k=1),
        )
...
class ContextExpert(nn.Module):
    ...
    def __init__(self, c, dils=(1, 2, 4)):
        super().__init__()
        self.pre = ConvBNAct(c, c, k=1)
        self.branches = nn.ModuleList([DWConvBNAct(c, k=3, dil=d) for d in dils])
        self.fuse = ConvBNAct(c * (len(dils) + 1), c, k=1)

    def forward(self, x):
        x0 = self.pre(x)
        xs = [x0] + [b(x0) for b in self.branches]
...
class MorphologyCueHead(nn.Module):
    ...
    def __init__(self, c, k_strip=7, dil=2, mid=32):
        ...
        # line-like: strip conv
        self.line_h = ConvBNAct(mid, mid, k=(1, k_strip), p=(0, k_strip // 2), g=mid)
        self.line_v = ConvBNAct(mid, mid, k=(k_strip, 1), p=(k_strip // 2, 0), g=mid)

        # blob-like: dilated DW conv
        self.blob = ConvBNAct(mid, mid, k=3, p=dil, g=mid, act=True)
        self.blob.conv.dilation = (dil, dil)
...
class RoadDefectMoE(nn.Module):
    ...
    def __init__(self, channels, num_experts=4, top_k=2, k_strip=7, dil=2, renorm_topk=True):
        ...
        # gating: [GAP(x)=B,C] concat [cue=B,2] -> B,num_experts
        self.gate_fc = nn.Sequential(
            nn.Linear(channels + 2, max(64, channels // 2)),
            nn.ReLU(inplace=True),
            nn.Linear(max(64, channels // 2), num_experts),
        )

        # optional residual scaling (稳定训练)
    ...
    def forward(self, x):
        ...
        # scatter -> sparse weights (B,E)
        sparse_w = torch.zeros_like(gate_scores)
...
# Utils
# -------------------------
def _auto_pad(k, dil=1):
    """Padding that keeps spatial size for stride=1."""
    if isinstance(k, tuple):
        # for tuple kernels, apply per-dim padding
        return (dil * (k[0] // 2), dil * (k[1] // 2))
    return dil * (k // 2)


# -------------------------
# Basic blocks (size-preserving)
# -------------------------
class ConvBNAct(nn.Module):
    ...
    def __init__(self, c1, c2, k=1, s=1, p=None, g=1, dil=1, act=True):
        super().__init__()
        if p is None:
            p = _auto_pad(k, dil=dil)
        self.conv = nn.Conv2d(c1, c2, k, s, padding=p, dilation=dil, groups=g, bias=False)
        self.bn = nn.BatchNorm2d(c2)
        self.act = nn.SiLU(inplace=True) if act else nn.Identity()
...
class DWConvBNAct(nn.Module):
    ...
    def __init__(self, c, k=3, s=1, dil=1, act=True):
        ...
        p = _auto_pad(k, dil=dil)  # ✅ key: padding = dil * (k//2) to keep size
        self.dw = ConvBNAct(c, c, k=k, s=1, p=p, g=c, dil=dil, act=act)
        self.pw = ConvBNAct(c, c, k=1, s=1, p=0, g=1, dil=1, act=act)
...
# -------------------------
# Experts (shape-specialized, all size-preserving)
# -------------------------
class CrackExpert(nn.Module):
    ...
    def __init__(self, c, k_strip=7):
        ...
        self.pre = ConvBNAct(c, c, k=1)
        # depthwise strip convs keep size due to correct padding
        self.h = ConvBNAct(c, c, k=(1, k_strip), s=1, p=_auto_pad((1, k_strip), dil=1), g=c)
        self.v = ConvBNAct(c, c, k=(k_strip, 1), s=1, p=_auto_pad((k_strip, 1), dil=1), g=c)
        self.mix = ConvBNAct(c, c, k=1)
...
class PotholeExpert(nn.Module):
    ...
    def __init__(self, c, dil=2):
        ...
        self.net = nn.Sequential(
            ConvBNAct(c, c, k=1),
            DWConvBNAct(c, k=3, dil=dil),  # ✅ keeps size
            ConvBNAct(c, c, k=1),
        )
...
class PatchRepairExpert(nn.Module):
    ...
    def __init__(self, c, k_big=9):
        ...
        self.net = nn.Sequential(
            ConvBNAct(c, c, k=1),
            ConvBNAct(c, c, k=k_big, s=1, p=_auto_pad(k_big, dil=1), g=c),  # DW big kernel, keep size
            ConvBNAct(c, c, k=1),
        )
...
class ContextExpert(nn.Module):
    ...
    def forward(self, x):
        ...
        for b in self.branches:
            y = b(x0)
            # ✅ should already match; keep an assert for "结构严谨"
            assert y.shape[-2:] == x0.shape[-2:], f"Context branch changed size: {y.shape[-2:]} vs {x0.shape[-2:]}"
            xs.append(y)
...
class MorphologyCueHead(nn.Module):
    ...
    def __init__(self, c, k_strip=7, dil=2, mid=32):
        ...
        # line-like
        self.line_h = ConvBNAct(mid, mid, k=(1, k_strip), s=1, p=_auto_pad((1, k_strip), 1), g=mid)
...
class RoadDefectMoE(nn.Module):
    ...
    def __init__(self, c1, c2, num_experts=4, top_k=2, k_strip=7, dil=2, renorm_topk=True):
        ...
        # If c1 != c2, project to c2 to match parse_model's channel tracking and keep graph consistent
    ...
    def forward(self, x):
        ...
        sparse_w = torch.zeros_like(gate_scores).scatter_(1, topk_idx, topk_scores)  # B,E

        # experts forward (all must keep H/W)
        outs = []
        for e in self.experts:
            y = e(x)
            assert y.shape[-2:] == (H, W), f"Expert changed size: {y.shape[-2:]} vs {(H, W)}"
            outs.append(y)
        outs = torch.stack(outs, dim=1)  # B,E,C,H,W

        y = (outs * sparse_w.view(B, self.num_experts, 1, 1, 1)).sum(dim=1)  # B,C,H,W
        return x + self.alpha * y
