...
class SGCLStrip(nn.Module):
    ...
    def forward_loss(self, feat: torch.Tensor, imgs: torch.Tensor, batch: dict, stride: float):
        ...
        roi_feat = roi_align(
            feat,
            rois,
            output_size=(cfg.roi_out, cfg.roi_out),
        ...
        roi_gray = roi_align(
            gray, rois, output_size=(cfg.mask_out, cfg.mask_out), spatial_scale=1.0, sampling_ratio=2, aligned=True
        )
        ...
        return aux * cfg.beta_aux, {
            "sgcl_n": float(pseudo_ds.shape[0]),
            "sgcl_focal": float(l_focal.detach().cpu()),
