...
def parse_model(d, ch, verbose=True):
    ...
    for i, (f, n, m, args) in enumerate(d["backbone"] + d["head"]):  # from, number, module, args
        ...
        if m in base_modules:
            ...
            c2_yaml = args[0]
            level = args[1] if len(args) > 1 else "P4"
            k_strip = args[2] if len(args) > 2 else 5
            alpha_init = args[3] if len(args) > 3 else 0.05
            beta_init = args[4] if len(args) > 4 else 0.2

            args = [c_list, c2_yaml, level, k_strip, alpha_init, beta_init]
            ...
            c1 = ch[f]  # 获取上一层的输出通道数
            # args 通常是 yaml 中的列表，我们假设 yaml 里不传复杂参数，使用默认 Config
            # 如果你想在 yaml 里传参，可以在这里解析 args 并更新 config
            cfg = SGCLStripConfig()
            # 示例：如果 yaml 写 [10, 0.25]，可以映射到 cfg 的某些字段
            # 这里简单起见，直接用默认配置
            args = [c1, cfg]
