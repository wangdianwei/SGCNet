...
def build_pseudo_centerline_from_gt(gt_labels, gt_bboxes, mask_gt, Hf, Wf, imgsz, crack_cls=(0, 1)):
    ...
    tgt = torch.zeros((B, 1, Hf, Wf), device=device)

    Hi = float(imgsz[0].item())
    Wi = float(imgsz[1].item())

    for b in range(B):
        for j in range(M):
            if not bool(mask_gt[b, j, 0].item()):
                continue
            c = int(gt_labels[b, j, 0].item())
            if c not in crack_cls:
                continue

            x1, y1, x2, y2 = gt_bboxes[b, j]
            # clip
            x1 = float(torch.clamp(x1, 0, Wi - 1).item())
            x2 = float(torch.clamp(x2, 0, Wi - 1).item())
            y1 = float(torch.clamp(y1, 0, Hi - 1).item())
            y2 = float(torch.clamp(y2, 0, Hi - 1).item())

            # map to feature coords
            x1f = int(max(0, min(Wf - 1, round(x1 / Wi * Wf))))
            x2f = int(max(0, min(Wf - 1, round(x2 / Wi * Wf))))
            y1f = int(max(0, min(Hf - 1, round(y1 / Hi * Hf))))
            y2f = int(max(0, min(Hf - 1, round(y2 / Hi * Hf))))
...
class v8DetectionLoss:
    ...
    def __init__(self, model, tal_topk: int = 10):  # model must be de-paralleled
        ...
        # # 添加FCLGS 损失
        self.model = model  # ✅ 保存模型引用（用来取 FCLGS 的缓存logit）
        # # ✅ 自动搜集所有 FCLGS（不写死层号）
        # self.fclgs_layers = []
        # for layer in model.model:
        #     # 只要有这两个缓存字段，就认为是 FCLGS
        #     if hasattr(layer, "last_m_crack_logit") and hasattr(layer, "last_m_rule_logit"):
        #         self.fclgs_layers.append(layer)

        # # ✅ 辅助loss权重（先给稳健默认值，不需要你调参）
        # self.w_line = 0.20
        # self.w_excl = 0.05
        # self.crack_cls = (0, 1)  # D00=0, D10=1
        # print("FCLGS layers found:", len(self.fclgs_layers))
        # self._sgcl_print_i = 0
    ...
    def __call__(self, preds: Any, batch: dict[str, torch.Tensor]) -> tuple[torch.Tensor, torch.Tensor]:
        ...
        if self.sgcl_module is not None and self.model.training:
            ...
            if hasattr(self.sgcl_module, "temp_feat"):
                ...
                # if aux_loss > 0:
                #     print(f"\n[SGCL Verify] Aux Loss: {aux_loss.item():.4f} | Stats: {stats}")
                # # ============================================
